...
func (a *Autopilot) buildServerState(inputs *nextStateInputs, srv raft.Server) ServerState {
	...
	if existing, found := a.state.Servers[srv.ID]; found {
		a.logger.Info("=====vishal: existing", "existing", existing)
	...
	a.logger.Info("=====vishal: previous", "health", state.Health.Healthy)
	a.logger.Info("======vishal: health","state.server.nodestatus", state.Server.NodeStatus)
	a.logger.Info("======vishal: health","state.stats.lastcontact", state.Stats.LastContact)
	a.logger.Info("======vishal: health","state.stats.lastterm", state.Stats.LastTerm)
	a.logger.Info("======vishal: health","state.stats.lastindex", state.Stats.LastIndex)
	a.logger.Info("======vishal: health","inputs.lastterm", inputs.LastTerm)
	a.logger.Info("======vishal: health","inputs.latestindex", inputs.LatestIndex)
	a.logger.Info("======vishal: health","inputs.config", inputs.Config)
	...
	a.logger.Info("=====vishal: after", "health", state.Health.Healthy)
